...
def test_model():
    ...
    try:
        ...
        assert False, (
            f"Model forward pass raised an exception: {e}"
        )  # either input not 4D tensor or input shape wrong != (N,1,28,28)
...
def test_parametrized_examples(batch_size):
    ...
    assert output_tensor.shape == (
        batch_size,
        10,
    ), f"Expected output shape ({batch_size}, 10), got {output_tensor.shape}"
